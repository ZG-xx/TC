...
npc_murky               100%    Pet Murky now starts dancing
EndContentData */
...
public:
    ...
    bool OnGossipHello(Player* pPlayer, Creature* pCreature)
    {
        ...
        if (pCreature->isCanTrainingAndResetTalentsOf(pPlayer))
            pPlayer->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_TRAINER, GOSSIP_HELLO_ROGUE1, GOSSIP_SENDER_MAIN, GOSSIP_OPTION_UNLEARNTALENTS);
        ...
        if (pPlayer->GetSpecsCount() == 1 && pCreature->isCanTrainingAndResetTalentsOf(pPlayer) && pPlayer->getLevel() >= sWorld->getIntConfig(CONFIG_MIN_DUALSPEC_LEVEL))
            pPlayer->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_TRAINER, GOSSIP_HELLO_ROGUE3, GOSSIP_SENDER_MAIN, GOSSIP_OPTION_LEARNDUALSPEC);
...
{
public:
    npc_murky() : CreatureScript("npc_murky") { }

    struct npc_murkyAI : public ScriptedAI
    {
        npc_murkyAI(Creature *c) : ScriptedAI(c) {}
        ...
        void UpdateAI(const uint32 uiDiff)
        {
            if (bActive) //only dance 10 seconds
            {
                if (uiDancing <= uiDiff)
                {
                    ...
                    uiDancing = 10000;
                    bActive = false;
                } else uiDancing -= uiDiff;
            } else {
                if (uiTimer <= uiDiff)
                {
                    if (x == me->GetPositionX() && y ==me->GetPositionY() && z == me->GetPositionZ()) //only dance when not moveing
                    {
                        bActive = true;
                        me->HandleEmoteCommand(EMOTE_ONESHOT_DANCE);
                    } else {
