...
public:
    ...
    struct npc_icefangAI : public ScriptedAI
    {
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            //until icefang uses waypoint movement
...
/*######
## npc_astrid_bjornrittar
######*/

class npc_astrid_bjornrittar : public CreatureScript
{
public:
    npc_astrid_bjornrittar() : CreatureScript("npc_astrid_bjornrittar") { }

    struct npc_astrid_bjornrittarAI : public ScriptedAI
    {
        npc_astrid_bjornrittarAI(Creature* pCreature) : ScriptedAI(pCreature) {}
        ...
        void MoveInLineOfSight(Unit *unit)
        {
            ...
            if (unit->ToCreature()->GetEntry() != 30468)
                return;

            if (me->IsWithinDist(unit, 20.0f, true))
            {
                ...
                if (!rider || !rider->ToPlayer())
                    return;

                rider->ToPlayer()->KilledMonsterCredit(29563, 0);
                rider->ExitVehicle();
            }
        }
    };
    ...
    CreatureAI *GetAI(Creature *creature) const
    {
        return new npc_astrid_bjornrittarAI(creature);
    }
};
