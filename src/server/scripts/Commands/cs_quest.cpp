...
public:
    ...
    ChatCommand* GetCommands() const
    {
        static ChatCommand questCommandTable[] =
        {
            ...
            { "autocomplite",   SEC_ADMINISTRATOR,  false, &HandleQuestAutocomplite,           "", NULL },
    ...
    static bool HandleQuestAutocomplite(ChatHandler* handler, const char* args)
    {
        if (!*args)
            return false;
        ...
        uint32 accountid = sObjectMgr->GetPlayerAccountIdByGUID(player->GetGUIDLow());
            
        uint32 sate = 0;
         
        WorldDatabase.PExecute("UPDATE quest_template SET method = '0' WHERE entry = '%u'",cId);
        WorldDatabase.PQuery("INSERT INTO quest_bugs VALUES commiter_guid=%u, commiter_accid=%u, quest_id=%u, comment=%c",player->GetGUIDLow(), accountid, cId, comment);
