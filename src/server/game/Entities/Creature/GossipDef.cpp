...
{
    LocaleConstant loc_idx = m_session->GetSessionDbLocaleIndex();

    char const* item_text = itemText ? sObjectMgr->GetTrinityString(itemText, loc_idx) : "";
    char const* box_text = boxText ? sObjectMgr->GetTrinityString(boxText, loc_idx) : "";
...
void PlayerMenu::SendQuestGiverQuestDetails(Quest const *pQuest, uint64 npcGUID, bool ActivateAccept)
{
    ...
    else
    {
        ...
        data << uint32(pQuest->XPValue(GetMenuSession()->GetPlayer())*sWorld->getRate(RATE_XP_QUEST));
    }

    // rewarded honor points. Multiply with 10 to satisfy client
    data << 10 * Trinity::Honor::hk_honor_at_level(GetMenuSession()->GetPlayer()->getLevel(), pQuest->GetRewHonorMultiplier());
