...
void AnticheatMgr::BuildReport(Player* player,uint8 reportType)
{
    ...
    if (sWorld->getBoolConfig(CONFIG_ANTICHEAT_BAN_ACCOUNT))
    {
        if (player->anticheatData.total_reports > sWorld->getIntConfig(CONFIG_ANTICHEAT_REPORTS_INGAME_NOTIFICATION))
        {
            std::string ban_date = "1h";
            std::string ban_author = "Античит";
            std::string ban_reason = "Чит!";
            std::string str = "";
            std::string ban_acc_name = "";
        
            uint32 account_id = sObjectMgr->GetPlayerAccountIdByGUID(player->GetGUIDLow());
        
            QueryResult result = LoginDatabase.PQuery("SELECT username FROM account WHERE id = '%u'", account_id);
            if (result)
                ban_acc_name = (*result)[0].GetString();
        
            if (!sAccountMgr->GetSecurity(sObjectMgr->GetPlayerAccountIdByGUID(player->GetGUID())) != SEC_PLAYER)
            {
                sWorld->BanAccount(BAN_ACCOUNT, ban_acc_name, ban_date, ban_reason, ban_author);
        
                str = "|cFFFFFC00[Античит]|cFF00FFFF[|cFF60FF00" + std::string(player->GetName()) + "|cFF00FFFF] Забанен на 1 час!";
    ...
    else
    {
        if (player->anticheatData.total_reports > sWorld->getIntConfig(CONFIG_ANTICHEAT_REPORTS_INGAME_NOTIFICATION))
        {
            if (!sAccountMgr->GetSecurity(sObjectMgr->GetPlayerAccountIdByGUID(player->GetGUID())) != SEC_PLAYER)
            {
                ...
                std::string str = "";
                str = "|cFFFFFC00[Античит]|cFF00FFFF[|cFF60FF00" + std::string(player->GetName()) + "|cFF00FFFF] Возможно читер!";
                WorldPacket data(SMSG_NOTIFICATION, (str.size()+1));
                data << str;
                sWorld->SendGlobalGMMessage(&data);
            }
...
void AnticheatMgr::JumpHackDetection(Player* player, MovementInfo movementInfo,uint32 opcode)
{
    if (player->anticheatData.lastOpcode == MSG_MOVE_JUMP && opcode == MSG_MOVE_JUMP)
    {
        ...
        sLog->outError("Jump Hack Detection Player LowGuid %u and name %s",player->GetGUIDLow(), player->GetName());
    }
}
...
void AnticheatMgr::WalkOnWaterHackDetection(Player* player, MovementInfo movementInfo)
{
    ...
    sLog->outError("Walk on Water Player LowGuid %u and name %s",player->GetGUIDLow(),player->GetName());
...
void AnticheatMgr::FlyHackDetection(Player* player, MovementInfo movementInfo)
{
    ...
    sLog->outError("FlyHack Player LowGuid %u and name %u",player->GetGUIDLow(),player->GetName());
...
void AnticheatMgr::SpeedHackDetection(Player* player,MovementInfo movementInfo)
{
    ...
    if (clientSpeedRate > speedRate)
    {
        ...
        sLog->outError("Speed Hack Player LowGuid %u and name %s",player->GetGUIDLow(),player->GetName());
    }
}